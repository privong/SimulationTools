...
parser.add_argument('-Etot', action='store_true', default=False,
                    help='Plot Etot (on 2nd y-axis)')
parser.add_argument('-Ekin', action='store_true', default=False,
                    help='Plot Ekin')
parser.add_argument('-Epot', action='store_true', default=False,
                    help='Plot Epot')
parser.add_argument('-Eint', action='store_true', default=False,
                    help='Plot Eint (normalized to the max value with time)')
parser.add_argument('-Erad', action='store_true', default=False,
                    help='Plot Erad')
parser.add_argument('-VirR', action='store_true', default=False,
                    help='Plot Virial Ratio')
"""
parser.add_argument('-allDot', action='store_true', default=False,
                    help='Plot all energetic time derivatives (overrides \
                          other options)')
parser.add_argument('-Udot', action='store_true', default=False,
                    help='Plot Udot internal')
"""
parser.add_argument('-tree', action='store_true', default=False,
                    help='Plot a treecode log format?')
parser.add_argument('-sph', action='store_true', default=True,
                    help='Plot a zeno sph log? (Default: True)')
parser.add_argument('-fixErad', action='store_true', default=False,
                    help='Fix the Erad and Etot budgeting due to a sign error \
                          (only needed for sphcode-1.3a runs)')
parser.add_argument('--density', action='store',
                    help='Overplot density vs time from this file (normalized \
                          to the maximum density)')
parser.add_argument('--output', action='store', default=False,
                    help='Specify the name of the output file (default is to \
                          change the log file suffix to .pdf)')
...
if args.sph:
    if args.Eint:
        plt.plot(time, Eint/Eint.max(), '--',
                 label='E$_{int}$/(E$_{int}$)$_{max}$')
    if args.fixErad:
        Etot = Etot-2*Erad
        Erad = -1*Erad
    if args.Erad:
    ...
    if args.density:
        plt.plot(t, rhoavg, "-", label='mean(rho) / mean(rho)$_{max}$',
                 color='green')
